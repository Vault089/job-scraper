...
def export_xls(jobs, date_str):
    ...
    # Upload XLS to Google Drive (Jobs Vietnam folder)
    xls_name = f"Vietnam Jobs - {datetime.now().strftime('%B %d, %Y')}.xlsx"
    upload_to_drive(xls_path, xls_name, DRIVE_FOLDER_JOBS_VIETNAM)
...
def generate_cover_letter(job):
    ...
    try:
        ...
        upload_to_drive(filepath, fname, DRIVE_FOLDER_COVER_LETTERS)
    except Exception as e:
        log(f"  ⚠️  Cover letter Drive upload failed: {e}")
    return filepath
